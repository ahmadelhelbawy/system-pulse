...
pub enum KillError {
    ...
    AccessDenied(u32),
    #[error("failed to terminate pid {0}")]
    Failed(u32),
}

/// Terminate a process by pid. Distinguishes not-found from access-denied.
///
/// Uses the OS primitive (`TerminateProcess` on Windows, `SIGTERM`/`SIGKILL`
/// on Unix) via `sysinfo`; never shells out.
pub fn kill_process(pid: u32) -> Result<(), KillError> {
    ...
    if process.kill() {
        ...
        Err(KillError::AccessDenied(pid))
    }
}
...
mod tests {
    ...
    fn kill_missing_process_is_not_found() {
        assert_eq!(kill_process(0), Err(KillError::NotFound(0)));
    }
    ...
    fn kill_spawned_process_succeeds_then_not_found() {
        ...
        assert_eq!(kill_process(pid), Ok(()));
        // Reap the zombie so the pid disappears from the process table.
        let _ = child.wait();
        sleep(Duration::from_millis(50));
        assert_eq!(kill_process(pid), Err(KillError::NotFound(pid)));
    }
}
